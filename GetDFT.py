...
def getDFT(data):
    ...
    entropyDFTData_ = np.empty((lengFrame))  # matrix containing DFT entropy data
    entropyDFTData_[:] = np.NaN
    entropyDFTData_ = entropyDFT(absDFTData_)
    x_.append(np.sum(entropyDFTData_[1:] * np.log(1 / entropyDFTData_[1:]), axis=0))  # normalize entropy
